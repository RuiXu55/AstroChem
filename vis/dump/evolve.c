...
static int f(realtype t, N_Vector n, N_Vector dn_o_dt, void *user_data);
/* Private function to check function return values */
static int check_flag(void *flagvalue, char *funcname, int opt);

/*============================================================================*/
int evolve(Real tend, Real dttry, Real atol)
{
  realtype t;
  ...
  flag = CVodeSVtolerances(cvode_mem, atol, vrtol);
  if (check_flag(&flag, "CVodeSVtolerances", 1)) return(1); 

  flag = CVSpgmr(cvode_mem,PREC_LEFT, 0);
  flag = CVSpilsSetGSType(cvode_mem, MODIFIED_GS);
    if(check_flag(&flag, "CVSpilsSetGSType", 1)) return(1);
  flag = CVBandPrecInit(cvode_mem,Chem->Ntot,Chem->Ntot-10,Chem->Ntot-10); //N, mu, ml
  ...
  while(Evln.t<tend)
  {
    ...
    //Evln.t *= 1.5;
    Evln.t = MIN(1.5*Evln.t, 1e5*OneYear+Evln.t);
    ...
    c1 = clock();
    if (((c1-c0)/CLOCKS_PER_SEC > 3600.0) || (status < 0))
      break;
  }
